...
class Touch{
	...
	addStyle(){
		...
		style.innerHTML = '.steer{ position:absolute; opacity: 0.5; width:80px; height:80px; border-radius: 80px; border:1px solid gray; top:calc(100% /2 - 80px);}' +
			'#leftSteer{left:100px;   } ' +
			'#rightSteer{right:100px; }';
	...
	onLeftChanged(x,y){
		...
		globalTouch.clearInfo();

		globalTouch.info("L<br/>rightDelta: "+globalTouch.rightDelta.x + " "+globalTouch.rightDelta.y +"<br/>" );
		globalTouch.info("leftDelta: "+globalTouch.leftDelta.x  + " "+ globalTouch.leftDelta.y  );
	}
	onRightChanged(x,y){
		...
		globalTouch.info("R<br/>rightDelta: "+globalTouch.rightDelta.x + " "+globalTouch.rightDelta.y +"<br/>" );
		globalTouch.info("leftDelta: "+globalTouch.leftDelta.x  + " "+ globalTouch.leftDelta.y  );
	}
